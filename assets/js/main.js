...
jQuery(function ($) {
    ...
    $(document).on('keyup', '#codecracker-form .field-email', function() {

        clearTimeout(timer);

        let url     = $('#codecracker-form').attr('action');
        let email   = $('#codecracker-form .field-email').val();
        let wpnonce = $('#codecracker_form_wpnonce').val();
        let data    = { 
            'email':email, 
            'step':'check_email', 
            'action':'codecracker_contact_form', 
            'section':'codecracker_form', 
            'codecracker_form_wpnonce':wpnonce
        };
            ...
            $.post(url, data, function(response) {
                try {
                    ...
                    if(obj.status == 'invalid' && obj.fields.email == true) {
                        $('#codecracker-form .field-email').removeClass('valid').addClass('invalid');
    
                    } else {
                        $('#codecracker-form .field-email').removeClass('invalid').addClass('valid');
                        $.valid_email = true;
                    }
    ...
    function next(button, group, show, hide) {
        ...
        $(document).on('click', button, function () {
            ...
            if(!$('#codecracker-form .field-email').length) {
                $.valid_email = true;
            }
    ...
    function submissionDone() {                
        if(codecracker_form.hasClass('redirect-sending')) {
            window.location.href = codecracker_form.data('redirect');
        } else {     
    ...
    // Submission
    var codecracker_form     = $('#codecracker-form');

    codecracker_form.submit(function(e) {
        e.preventDefault();
        ...
        var url = codecracker_form.attr('action');

        $.ajax({
            type: 'POST',
            url: url,
            data: codecracker_form.serialize(),
            success: function() {                
                submissionDone();
            }
        })
...
jQuery(function ($) {
    ...
    sendForm('#codecracker-simple-form');
    sendForm('#codecracker-subscribe');
})
